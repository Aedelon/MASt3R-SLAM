...
// Gauss-Newton point alignment kernel for MASt3R-SLAM on Apple Silicon
// Computes per-point residuals and Jacobians for 3D point alignment
...
// Kernel: Compute per-point Jacobian contribution for 3D point alignment
// This is similar to ray alignment but uses raw 3D coordinates without normalization
kernel void gn_points_jacobian_kernel(
    ...
    device float* JtJ_i [[buffer(8)]],               // [num_edges * num_pts, 28]
    ...
    device float* JtJ_ij [[buffer(12)]],             // [num_edges * num_pts, 49]
    ...
    // Residual: direct 3D error (no ray normalization)
    float3 err = Xj_Ci - Xi;

    // Weight based on distance for scale-invariant error
    float dist = length(Xi) + EPS;
    float scale_factor = 1.0f / dist;  // Weight inversely with distance

    // Apply weights
    float sqrt_w = sigma_inv * sqrt(q_conf) * scale_factor;
    float w[3];
    ...
    // Build Jacobians (same structure as ray kernel)
    float Ji[3][7], Jj[3][7];
