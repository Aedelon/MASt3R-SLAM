...
"""Metal GPU runner for point-based Gauss-Newton optimization."""
...
class GNPointsMetalRunner:
    """Runs point-based Gauss-Newton optimization using Metal GPU acceleration."""
    ...
    def _load_shaders(self):
        ...
        # Load points kernel
        shader_path = Path(__file__).parent / "shaders" / "gauss_newton_points.metal"
        ...
        func = library.newFunctionWithName_("gn_points_jacobian_kernel")
        if func:
            ...
            self.pipelines["gn_points_jacobian_kernel"] = pipeline
        ...
        gn_library, _ = self.device.newLibraryWithSource_options_error_(gn_source, options, None)
        if gn_library:
    ...
    def gauss_newton_points(
        ...
        """Run point-based Gauss-Newton optimization with Metal GPU acceleration.
        ...
        sigma_inv = np.float32(1.0 / sigma_point)

        # Create input buffers
        Twc_buf = self._create_buffer(Twc_work)
        Xs_buf = self._create_buffer(Xs.astype(np.float32))
        Cs_buf = self._create_buffer(Cs.astype(np.float32))
        ...
        jac_pipeline = self.pipelines.get("gn_points_jacobian_kernel")
        update_pipeline = self.pipelines.get("pose_update_kernel")

        if jac_pipeline is None:
            raise RuntimeError("gn_points_jacobian_kernel not compiled")
...
# Global instance
_gn_points_runner: Optional[GNPointsMetalRunner] = None


def get_gn_points_runner() -> GNPointsMetalRunner:
    """Get or create global GN Points Metal runner."""
    global _gn_points_runner
    if _gn_points_runner is None:
        _gn_points_runner = GNPointsMetalRunner()
    return _gn_points_runner
